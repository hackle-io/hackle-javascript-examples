...
function promiseWithTimeout<T>(callback, { timeoutMillis = 5000, onTimeout }) {
...
class HackleWebViewClient extends Emitter<{
  ...
  variation(experimentKey): Promise<string> {
    const id = this.#createId();
...
class HackleWebOnlyClient extends Emitter<{
  ...
  setUser(user) {
    this.emit("user-updated", JSON.stringify(user));
  ...
  variation(experimentKey): Promise<string> {
    return new Promise((resolve) => {
      const variationResult = this.client.variation(experimentKey);
      setTimeout(() => {
...
class WebViewRemoteConfig {
  constructor(private readonly configFetcher) {
